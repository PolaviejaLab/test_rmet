...
(function (RMET) {
    ...
    RMET.signal_central_waiting = signal_central_waiting;
    /**
    ...
    RMET.signal_central_complete = signal_central_complete;
    /**
    ...
    RMET.signal_peripheral_complete = signal_peripheral_complete;
})(RMET || (RMET = {}));
var RMET;
(function (RMET) {
    /**********************************************
    ...
    RMET.get_stimuli = get_stimuli;
})(RMET || (RMET = {}));
var RMET;
(function (RMET) {
    var test_RMET = function ($scope, $http, $cookies, $controller) {
        ...
        */
        ...
        $scope.set_final = function (value) {
            ...
            // Send signal        
            if (value)
                RMET.signal_peripheral_complete(Channel, $scope.trial['id'], $scope.withinGroupId);
        };
        /************************************************
        ...
        $scope.set_trial = function (trialNr) {
            ...
            if ($scope.get_mode() == $scope.Mode_Central)
                RMET.signal_central_waiting(Channel, $scope.trial['id']);
            return true;
        };
        /**
        ...
        */
        ...
        Channel
            ...
            if ($scope.get_ready_count() == 3) {
                RMET.signal_central_complete(Channel, $scope.trial['id']);
                $scope.next();
            }
        });
        // Send status
         ...
         */
        RMET.get_descriptions($scope.resources, 'en').subscribe(function (descriptions) { $scope.descriptions = descriptions; }, function (error) { console.log("Could not load descriptions:", error); }, function () { });
        /**
         * Get stimuli and start experiment
         */
        RMET.get_stimuli($scope.resources, 'en').subscribe(function (trial) { $scope.trials.push(trial); }, function (error) { console.log("Could not load stimuli:", error); }, function () { $scope.$apply(function () { $scope.set_trial(0); }); });
    };
    experimentFrontendControllers.controller('RMET', ['$scope', '$http', '$cookies', '$controller', test_RMET]);
})(RMET || (RMET = {}));
